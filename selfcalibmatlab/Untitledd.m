...
% error in solution
[corrs, IMS, P,ks, F] = readCorrsOxford('wadham', 0,0);
...
for i=0:0.001:0.1
    ...
    %noise
    [corrs, IMS, P,ks, F] = readCorrsOxford('wadham', noise,1);
    x1=[ks{1,1}(1,1) ks{1,1}(1,3) ks{1,1}(2,3) ks{1,1}(2,2)];
    x2=[w w/2 h/2 w];
    
    
    v1(count)=computerEssentialErrorSVDNFramesWeighted(x1,F(1));
...
subplot(1,2,1);
plot(t,z1);
title('residuals versus error in focal length');
xlabel('error in focal length');
ylabel('residual');
subplot(1,2,2)
plot(tv,v1);
title('residuals versus percentage error in fundamental matrix');
xlabel('error in fundamental matrix');
ylabel('residual');